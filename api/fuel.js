...
module.exports = async function handler(req, res) {
  ...
  if (!clientId || !clientSecret) {
    console.error('[fuel] Missing env vars: FUEL_CLIENT_ID / FUEL_CLIENT_SECRET');
  ...
  try {
    ...
    console.log('[fuel] Token response body:', body.slice(0, 200));
    if (status !== 200) {
    ...
    console.log('[fuel] Token obtained. Starts with:', token.slice(0, 8) + '...');
  } catch (err) {
  ...
  try {
    ...
    console.log('[fuel] Prices response (first 200 chars):', body.slice(0, 200));
    if (status !== 200) {
